...
export default function App() {
  ...
  const { photos: googlePhotos, loading: googleLoading } = usePlacePhotos(activeLocation.query, activeLocation.name);
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden font-sans bg-zinc-950 text-zinc-100">
        ...
        <div className="flex items-center gap-4 flex-1 justify-end">
          <div className="relative flex-1 max-w-[200px] sm:max-w-none">
            ...
            <input 
              ...
              className="w-full bg-zinc-800/50 border border-zinc-700 rounded-full py-1.5 pl-9 pr-4 text-xs focus:outline-none focus:ring-2 focus:ring-orange-500/50 transition-all"
            />
          </div>
        </div>
      </header>
      ...
      <div className="md:hidden bg-zinc-900/50 border-b border-zinc-800 px-4 py-3 overflow-x-auto no-scrollbar flex gap-2 shrink-0">
        {categories.map((cat) => (
          <button
            key={cat.id}
            onClick={() => setFilter(cat.id)}
            className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 whitespace-nowrap ${
              filter === cat.id 
              ? 'bg-zinc-100 text-zinc-950' 
